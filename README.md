...
ReefSafe is an evidence-led screening tool for deciding which monitored Malaysian islands should receive earlier reef field checks. The production model uses Reef Check observations. NOAA thermal-stress records and OpenDOSM tourism/GDP data remain descriptive context only.
...
- `output/TeamName_Datathon2026_Report.pdf` — verified 10-page report with factor diagnostics and assumptions.
...
Raw files are retained under `data/raw/`: Reef Check Malaysia annual survey reports, NOAA Coral Reef Watch station series, OpenDOSM structured datasets, island coordinate mappings, the DOSM Datathon 2026 booklet, and the Department of Marine Park Malaysia economic-valuation report.
